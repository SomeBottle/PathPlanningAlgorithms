...
实现的时候为了能让 JPS 算法在右上角（图中）这个地方能继续沿着红色箭头这个方向搜索，实际上我只是**暂时记录了**红色框代表的绕路结点并**修正了到达右上角的路径长度**罢了。    
...
因此，需要**把绕路结点也加入** `open_list`，并强制其搜索方向平行于原本的搜索方向。  
...
2. 在往一个方向寻找跳点的过程中（`_jump` 方法），如果碰到对角障碍物，如下这种情况：  
    ...
    > 黄色箭头为一个跳点搜索方向；`#JUMP_POINT` 标记的为跳点；`#VIRTUAL` 标记的为实不存在的结点；`#DETOUR` 为绕路结点；黑色块为障碍物；黄色块为 `open_list` 结点；红色块为最后求解得到的路径。

    图中这里的绕路结点 `#DETOUR` 左方是没有实际的结点的，需要在其左侧添加一个父节点，并将这个父节点的父链接连接到路径（黄色箭头这条线）上的上一个跳点。  
